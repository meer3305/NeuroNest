...
def create_animated_play_video(prompt, output_path, duration=3.0, fps=24):
    ...
    for frame_num in range(total_frames):
        img = Image.new('RGB', (720, 1280), color=(254, 243, 199))
        draw = ImageDraw.Draw(img)
...
def create_animated_read_video(prompt, output_path, duration=3.0, fps=24):
    """Create an animated video of a character reading"""
    frames = []
    total_frames = int(duration * fps)
...
def create_animated_clean_video(prompt, output_path, duration=3.0, fps=24):
    ...
    for frame_num in range(total_frames):
        img = Image.new('RGB', (720, 1280), color=(240, 253, 244))
        draw = ImageDraw.Draw(img)
...
def create_animated_wake_up_video(prompt, output_path, duration=3.0, fps=24):
    ...
    for frame_num in range(total_frames):
        img = Image.new('RGB', (720, 1280), color=(255, 248, 220))
        draw = ImageDraw.Draw(img)
        
        progress = frame_num / total_frames
        
        sun_y = 1000 - int(progress * 300)
        sun_x = 360
        ...
        eye_open_progress = max(0, (progress - 0.3) / 0.7)
        ...
        smile_progress = max(0, (progress - 0.5) / 0.5)
        ...
        stretch_progress = max(0, (progress - 0.4) / 0.6)
...
def create_animated_dress_video(prompt, output_path, duration=3.0, fps=24):
    ...
    for frame_num in range(total_frames):
        img = Image.new('RGB', (720, 1280), color=(248, 250, 252))
        draw = ImageDraw.Draw(img)
        ...
        body_visibility = min(1.0, progress * 1.5)
...
def create_animated_bath_video(prompt, output_path, duration=3.0, fps=24):
    ...
    for frame_num in range(total_frames):
        img = Image.new('RGB', (720, 1280), color=(219, 234, 254))
        draw = ImageDraw.Draw(img)
        ...
        water_level = max(20, min(water_level, tub_height - 20))
        ...
        char_head_y = tub_y + max(water_level - 30, 10)
        ...
        eye_height = max(2, int(12 * eye_relax))
        ...
        for i in range(8):
            ...
            bubble_size = max(1, 2 + math.sin(progress * 12 * math.pi + i) * 3)
...
def create_animated_eat_breakfast_video(prompt, output_path, duration=3.0, fps=24):
    ...
    for frame_num in range(total_frames):
        img = Image.new('RGB', (720, 1280), color=(255, 253, 208))
        draw = ImageDraw.Draw(img)
        ...
        chew_size = 12 + math.sin(progress * 8 * math.pi) * 6
        ...
        arm_motion = math.sin(progress * 6 * math.pi) * 20

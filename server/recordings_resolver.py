...
Covers ALL flashcard step titles and descriptions used in the demo store / default routines.
"""
import os
import shutil
import time
...
# Keyword (in prompt) -> filename in server/recordings (longer phrases matched first)
PROMPT_TO_VIDEO = {
    # ── Brush Teeth ──────────────────────────────────────────────────────────
    ...
    # ── Wake Up ──────────────────────────────────────────────────────────────
    "open eyes and stretch":          "waking up.mp4",
    "open your eyes and sit up":      "waking up.mp4",
    "open your eyes and stretch":     "waking up.mp4",
    "sit up in bed":                  "waking up.mp4",
    "open your eyes":                 "waking up.mp4",
    "waking up":                      "waking up.mp4",
    "wake up":                        "waking up.mp4",
    "morning":                        "waking up.mp4",
    "stretch":                        "waking up.mp4",
    "alarm":                          "waking up.mp4",
    "wake":                           "waking up.mp4",
...
def resolve_recording(prompt: str, out_dir: str, filename_prefix: str = "animation") -> str | None:
    """
    If a matching MP4 exists in server/recordings, copy it to out_dir and return destination path.
    Returns None if no match found (caller can fall back to YouTube or other sources).
    """
    ...
    # 2) Scan recordings folder: match by filename tokens
    try:
